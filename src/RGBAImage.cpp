...
RGBAValue RGBAImage::getTexel(float u, float v, bool bilinearFiltering) {
    ...
    float floatRow = (v * static_cast<float>(height - 1));
    int intRow = static_cast<int>(floatRow);
    float floatCol = (u * static_cast<float>(width - 1));
    int intCol = static_cast<int>(floatCol);
...
bool RGBAImage::readPPM(std::istream& inStream) {
    ...
    if (newWidth < 1 || newWidth > MAX_IMAGE_DIMENSION || newHeight < 1 || newHeight > MAX_IMAGE_DIMENSION) {
        // bad sizes
        std::cerr << "RGBA image dimensions " << newWidth << " x " << newHeight << " were outside range of 1 - "
                << MAX_IMAGE_DIMENSION << std::endl;
        return false;
    }
...
void RGBAImage::clear(const RGBAValue& color) {
    for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
            (*this)[row][col] = color;
        }
